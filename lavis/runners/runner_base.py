...
class RunnerBase:
    ...
    @property
    def save_ckpt_every(self):
        return self.config.run_cfg.get("save_ckpt_every", 1)
    ...
    def setup_visualizer(self):
        vis_data_dir = self.output_dir / "vis_data"
    ...
    def train(self):
        ...
        for cur_epoch in range(self.start_epoch, self.max_epoch):
            ...
            if not self.evaluate_only:
                ...
                train_stats = self.train_epoch(cur_epoch, self.max_epoch - cur_epoch - 1)
                ...
                for split_name in self.valid_splits:
                    ...
                    if val_log is not None:
                        if is_main_process():
                            ...
                            if agg_metrics > best_agg_metric and split_name in ["val", "test"]:
                                best_epoch, best_agg_metric = cur_epoch, agg_metrics

                                self._save_checkpoint(cur_epoch, is_best=True)
                            elif cur_epoch % self.save_ckpt_every == 0:
                                self._save_checkpoint(cur_epoch, is_best=False)

                            val_log.update({"best_epoch": best_epoch})
                            for k, v in val_log.items():
                                self.visualizer.add_scalar(f'{split_name}/{k}', v, cur_epoch)
    ...
    def evaluate(self, cur_epoch="best", skip_reload=False):
        ...
        if len(self.test_splits) > 0:
            for split_name in self.test_splits:
                ...
                if cur_epoch != "best":
                    for k, v in test_logs[split_name].items():
                        self.visualizer.add_scalar(f'{split_name}/{k}', v, cur_epoch)

            return test_logs

    def train_epoch(self, epoch, remain_epochs):
        # train
        self.model.train()

        return self.task.train_epoch(
            epoch=epoch,
            remain_epochs=remain_epochs,
            model=self.model,
